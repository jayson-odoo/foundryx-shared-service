...
def test_assign_and_remove_users(client, session_factory):
    _make_active_user(session_factory, "assignee@foundryx.io", "pw12345678")
    role = client.post(
    ...
    target = next(u for u in assignable if u["email"] == "assignee@foundryx.io")
    ...
    assert any(u["email"] == "assignee@foundryx.io" for u in assigned)
    ...
    assert not any(u["email"] == "assignee@foundryx.io" for u in assigned2)
...
def test_no_permission_user_gets_403(client, session_factory):
    _make_active_user(session_factory, "noperm@foundryx.io", "pw12345678")
    headers = _auth(client, email="noperm@foundryx.io", password="pw12345678")
