...
def test_theme_css_renders_overrides_only(client):
    ...
    assert "--foundryx-primary: #0050ff;" in css
...
def test_frontend_defaults_parity():
    ...
    from app.branding.token_whitelist import FOUNDRYX_DEFAULTS, TOKEN_DEFS

    ts_path = (
        Path(__file__).resolve().parents[2]
        / "service_frontend" / "lib" / "branding-tokens.ts"
    )
    ...
    # Defaults: the FOUNDRYX_DEFAULTS block, split into light/dark sections.
    block = re.search(
        r"export const FOUNDRYX_DEFAULTS[^=]*=\s*\{(.*?)\n\};", src, re.DOTALL
    ).group(1)
    ...
    assert ts_light == FOUNDRYX_DEFAULTS["light"]
    assert ts_dark == FOUNDRYX_DEFAULTS["dark"]
