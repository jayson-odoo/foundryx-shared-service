...
def test_new_permissions_granted_to_existing_tenant_admin_on_update(client, session_factory):
    ...
    # Plan 29 S1 bumped the manifest to 0.5.0, plan 28 to 0.6.0 and plan 33 S1
    # to 0.7.0 - this test pins "the CURRENT manifest version", not a fixed
    # string (updated the same way every prior bump updated it before).
    assert module.version == "0.7.0"
