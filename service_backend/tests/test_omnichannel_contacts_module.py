...
def test_new_permissions_granted_to_existing_tenant_admin_on_update(client, session_factory):
    ...
    # Plan 29 S1 bumped the manifest to 0.5.0, plan 28 to 0.6.0, plan 31 to
    # 0.7.0, plan 33 S1 to 0.8.0, plan 32 to 0.9.0, plan 34 S1 (A7b) to
    # 0.10.0 and review round 2 (N-new-4, no schema change) to 0.10.1 - this
    # test pins "the CURRENT manifest version", not a fixed string (updated
    # the same way every prior bump updated it).
    assert module.version == "0.10.1"
    assert new_state.installed_version == "0.10.1"
    db2.close()
