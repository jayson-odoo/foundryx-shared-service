...
def test_create_user_is_invited_and_listed(client):
    res = client.post(
        ...
        json={"name": "New Hire", "email": "new.hire@foundryx.io", "roleIds": [], "status": "ACTIVE"},
    )
    ...
    assert any(u["email"] == "new.hire@foundryx.io" for u in listed)
...
def test_invited_status_not_overwritten_by_update(client):
    created = client.post(
        "/users",
        headers=_auth(client),
        json={"name": "Pending", "email": "pending@foundryx.io", "roleIds": [], "status": "ACTIVE"},
    ).json()
    res = client.patch(
...
def test_set_password_activates_invited_user(client, session_factory):
    created = client.post(
        "/users",
        headers=_auth(client),
        json={"name": "Invitee", "email": "invitee@foundryx.io", "roleIds": [], "status": "ACTIVE"},
    ).json()
    ...
    login = client.post(
        "/auth/login", json={"email": "invitee@foundryx.io", "password": "NewPass1!"}
    )
