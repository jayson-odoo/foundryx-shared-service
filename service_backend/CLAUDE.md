# Backend - scope-local rules (`service_backend/`)

> **Shared-service fork:** this is the Foundryx Shared Service Platform backend (forked from Foundryx EMS). The EMS domain modules (`modules/ems`, `modules/crm`, `modules/finance`) are removed; `modules/omnichannel` is the first **Service**. The module loader is manifest-driven (`modules/*/manifest.json`), so the live module set = whatever dirs exist. All rules below apply unchanged.
>
