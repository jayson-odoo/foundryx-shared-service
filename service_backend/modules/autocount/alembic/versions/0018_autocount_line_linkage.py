...
* adds the six enabled, not-required ``FromSO*``/``FromPO*`` line mapping
...
* when a statement is byte-identical to the OLD preset text (own company's
  ...
  AC-06-13 are appended to ``result_columns`` - the compared-column set a
  paged run's change-detection hash derives from ``result_columns``, so
  without this the newly-selected link columns never enter the hash and a
  document whose only change is a newly populated link would never
  re-stage. A customised statement is left untouched with a WARNING naming
  the config id; the mapping rows above still land regardless.
...
Revision ID: 0018_autocount_line_linkage   (26 chars <= 32)
