...
def update_tenant(db: Session, tenant_id: str, from_version: str) -> None:
    ...
    **0.1.0 → 0.2.0 (masters).** Two things existing tenants need, and neither is
    ...
    # sprint-5/04 (Sorento contract 2.1): a mapping row saved before
    # ``credit_limit`` left ``CanonicalCustomer.SINK_FIELDS`` is now inert and
    # would trip the mapping PUT guard on the next save - disable it here.
