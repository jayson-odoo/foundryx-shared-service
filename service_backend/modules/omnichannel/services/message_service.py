...
class MessageService:
    ...
    def send_message(
        ...
        # Resolve the reply target up front (also threads the WhatsApp quote).
        quoted: Optional[ConversationMessage] = None
        if payload.replyToMessageId:
            quoted = self.repo.get_message(payload.replyToMessageId, tenant_id)
        ...
        if message_type == "TEMPLATE":
            ...
            # Meta rejects a parameter-count mismatch — validate before sending
            # rather than shipping a body with blank placeholders.
            ...
            send_kwargs: Dict[str, Any] = {"template": template_payload}
        else:
        ...
        try:
            result = adapter.send(
                ...
                to,
                context_message_id=(quoted.external_message_id if quoted else None),
                **send_kwargs,
            )
        ...
        row = ConversationMessage(
            ...
            message_type=message_type if message_type == "TEMPLATE" else "TEXT",
            body=body,
