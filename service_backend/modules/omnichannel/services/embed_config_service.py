...
# Plan 34 / A7b S1 (D-A7B-13) - `_validate_origin`/`_validate_origins`/
# `InvalidOrigin` MOVED verbatim into the shared `origins.py` (the web chat
# channel reuses them); re-imported here so this module's own callers (this
# file's other methods, the `embed_config` router, `test_omnichannel_embed_
# config.py`) see ZERO change - a pure move.
from ..origins import InvalidOrigin, _validate_origin, _validate_origins  # noqa: F401
