...
class Settings(BaseSettings):
    ...
    #
    # This is the APPLICATION default (1, local dev). The DEPLOYED default
    # is `docker-compose.yml`'s own `AUTOCOUNT_SINK_CONCURRENCY` (2 since PR
    # #58, per Sorento's #710 capacity measurement), overridable per
    # environment via `.env`; `feat/sink-concurrency-ui` then lets a single
    # tenant's Sorento connection override BOTH from its own edit form
    # (`SorentoSink._resolve_concurrency`). Bounded 1..4 at every level.
