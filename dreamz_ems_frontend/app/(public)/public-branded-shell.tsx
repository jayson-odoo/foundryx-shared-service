...
export interface PublicBrandedShellProps {
  children: ReactNode;
  /** Server-resolved branding (layout) — first paint, no Dreamz flash. */
 ...
 * NAME when it has no logo); an unbranded host shows the Dreamz wordmark.
 */
...
function BrandMark({ branding }: { branding: PublicBranding }) {
  ...
  // image — fall back to its NAME (or the Dreamz mark only when unbranded).
  ...
  return (
    <Image
      src="/media/dreamz/dreamz-logo.png"
      alt="Dreamz EMS"
      width={120}
      height={32}
      className="h-7 w-auto object-contain"
