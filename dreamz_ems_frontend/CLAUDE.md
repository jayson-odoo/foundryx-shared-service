# Frontend — scope-local rules (`dreamz_ems_frontend/`)
...
- **White-label** — never render "Dreamz" to tenants; branded tenant w/o logo → its name.
- **Truncation** — `ClampedText`/`OverflowPills`, never bare `truncate`/`line-clamp`.
- **No `<style>` / raw CSS** — Tailwind/Metronic utilities only. Brand tokens = `css/dreamz-tokens.css`.
