...
async function searchAndSettle(page: Page, slug: string) {
  ...
  await expect(page.getByText('Dreamz EMS')).toHaveCount(0);
...
test.describe('Platform Console — Tenants (live stack)', () => {
  test('operator sees the console; seeded tenants listed', async ({ page }) => {
    ...
    await page.getByPlaceholder('Search tenants…').fill('Dreamz');
    await expect(page.getByText('Dreamz EMS')).toBeVisible();
    await expect(
      rowByName(page, 'Dreamz Platform').getByText('Platform', { exact: true }),
    ).toBeVisible();
  });
  ...
  test('platform tenant row has no lifecycle actions', async ({ page }) => {
    ...
    await page.getByPlaceholder('Search tenants…').fill('Dreamz Platform');
    const platform = rowByName(page, 'Dreamz Platform');
