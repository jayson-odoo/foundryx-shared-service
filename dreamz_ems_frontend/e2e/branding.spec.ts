...
test.describe('Tenant Branding (live stack)', () => {
  test('tenant brands the workspace: logo + slogan + theme, applied live and on the sign-in page', async ({
    ...
    await expect
      .poll(() =>
        page.evaluate(() =>
          document.documentElement.style.getPropertyValue('--dreamz-primary'),
        ),
      )
      .toBe(PURPLE);
    ...
    // Pre-auth sign-in page: tenant logo + slogan + themed button, no Dreamz leak.
  ...
  test('unbranded tenant gets stock Dreamz branding on its sign-in page', async ({
    page,
    request,
  }) => {
    ...
    await expect(page).toHaveTitle(/Dreamz EMS/);
  });
  ...
  test('operator edits a tenant’s branding from the console Branding tab', async ({
    ...
    // operator's console keeps stock Dreamz theming (edit targeted the OTHER
