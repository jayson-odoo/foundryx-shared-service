 ...
 * platform.dreamzems.com + acme → https://acme.dreamzems.com.
 */
export function tenantUrl(slug: string, location: Location): string {
  ...
  // (localhost, dreamzems.com) are already the base.
 ...
 * (`acme.dreamzems.com` → `acme`) or of `<slug>.localhost` (dev), unless it's
...
export function deriveTenantSlug(hostname: string): string {
  ...
  // subdomain + domain + tld host (acme.dreamzems.com).
