...
describe('whitelist + defaults', () => {
  it('defaults cover every whitelisted key in both themes', () => {
    for (const d of TOKEN_DEFS) {
      expect(DREAMZ_DEFAULTS.light[d.key], `light.${d.key}`).toBeDefined();
      expect(DREAMZ_DEFAULTS.dark[d.key], `dark.${d.key}`).toBeDefined();
    }
  });

  it('every default is a valid color', () => {
    for (const theme of ['light', 'dark'] as const) {
      for (const v of Object.values(DREAMZ_DEFAULTS[theme])) {
...
describe('validateTokens', () => {
  ...
  it('keeps true diffs only', () => {
    const { tokens } = validateTokens({
      light: { primary: '#0050ff', success: DREAMZ_DEFAULTS.light['success'] },
      dark: {},
    });
...
describe('derivation + emission', () => {
  ...
  it('overrideVars emits only overridden vars + derived transparents', () => {
    ...
    expect(vars).toEqual({
      '--dreamz-primary': '#0050ff',
      '--dreamz-primary-transparent': 'rgba(0, 80, 255, 0.2)',
    });
    expect(
  ...
  it('tokensToCss renders :root and .dark blocks for the overridden side only', () => {
    ...
    expect(css).toContain('--dreamz-primary: #4d82ff;');
  });
  ...
  it('effectiveTokens merges overrides over defaults', () => {
    ...
    expect(t['success']).toBe(DREAMZ_DEFAULTS.light['success']);
  });
});
