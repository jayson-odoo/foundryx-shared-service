 ...
 * Mirrors the Layer-1 `--dreamz-*` primitives in `css/dreamz-tokens.css`; the
...
export const DREAMZ_DEFAULTS: BrandingTokens = {
  light: {
    primary: '#ff5a00',
...
export function effectiveTokens(
  ...
  return { ...DREAMZ_DEFAULTS[theme], ...(tokens?.[theme] ?? {}) };
}
 ...
 * Dreamz default are normalized away so the stored doc stays a true diff.
 */
export function validateTokens(input: unknown): TokenValidation {
  ...
  for (const theme of ['light', 'dark'] as const) {
    ...
    for (const [key, value] of Object.entries(
      ...
      if (normalized !== normalizeHex(DREAMZ_DEFAULTS[theme][key])) {
