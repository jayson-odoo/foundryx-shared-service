...
describe('deriveTenantSlug', () => {
  it('takes the first label of a 3-label host', () => {
    expect(deriveTenantSlug('acme.dreamzems.com')).toBe('acme');
  });

  it('strips the port', () => {
    expect(deriveTenantSlug('acme.dreamzems.com:3001')).toBe('acme');
  });
  ...
  it('falls back for bare domains (no subdomain)', () => {
    expect(deriveTenantSlug('dreamzems.com')).toBe('default');
  });

  it('falls back for reserved infra subdomains', () => {
    expect(deriveTenantSlug('www.dreamzems.com')).toBe('default');
    expect(deriveTenantSlug('api.dreamzems.com')).toBe('default');
  });
  ...
  it('platform/default subdomains resolve (real tenants, not infra aliases)', () => {
    expect(deriveTenantSlug('platform.dreamzems.com')).toBe('platform');
    expect(deriveTenantSlug('default.dreamzems.com')).toBe('default');
  });
});
...
describe('tenantUrl', () => {
  ...
  it('swaps the subdomain on the operator host', () => {
    ...
    expect(tenantUrl('acme', loc('platform.dreamzems.com', 'https:'))).toBe(
      'https://acme.dreamzems.com',
    );
  });

  it('treats bare hosts as the base domain', () => {
    expect(tenantUrl('acme', loc('localhost:3001'))).toBe('http://acme.localhost:3001');
    expect(tenantUrl('acme', loc('dreamzems.com', 'https:'))).toBe(
      'https://acme.dreamzems.com',
    );
  });
});
