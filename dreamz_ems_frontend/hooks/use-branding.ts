...
type PublicState = {
  /** null until the first fetch resolves (avoids a Dreamz→tenant flash where possible). */
...
function fetchPublicBranding(): Promise<void> {
  publicPromise ??= brandingService
    ...
    .catch(() => {
      // Consumption surfaces degrade to Dreamz defaults — never block render.
...
export interface UseTenantBrandingResult {
  /** Resolved branding — Dreamz defaults while loading / when unbranded. */
