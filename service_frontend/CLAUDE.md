...
- **Responsive** - verify 375px AND 1280px (`agent-browser` CLI to set the viewport and screenshot both). Side-by-side panels stack on mobile.
...
## Tests
`npm test` (Vitest, config `vitest.config.mts`). `[E2E]` = a recorded `agent-browser` CLI run, REAL clicks vs the live stack (backend up + seeded). Lint gates the prod build (`npx eslint` before `npm run build`): no statement-position ternaries, no unused imports, `Array.from` to spread a Set.
