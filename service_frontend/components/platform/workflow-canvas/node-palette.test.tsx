...
function renderPalette(props: Partial<React.ComponentProps<typeof NodePalette>> = {}) {
  return render(
    <DndContext onDragEnd={() => {}}>
      <NodePalette hasTrigger={false} disabled={false} onAdd={vi.fn()} {...props} />
    </DndContext>,
  );
}
...
describe('NodePalette module filtering', () => {
  it('hides the omnichannel trigger + actions when the module is inactive', () => {
    ...
    fireEvent.change(screen.getByTestId('palette-search'), { target: { value: 'omnichannel' } });

    expect(screen.queryByTestId('palette-omnichannel.message_received')).not.toBeInTheDocument();
    expect(screen.queryByTestId('palette-omnichannel.get_contact')).not.toBeInTheDocument();
    expect(screen.queryByTestId('palette-omnichannel.send_message')).not.toBeInTheDocument();
  ...
  it('shows the omnichannel trigger + actions once the module is active', () => {
    ...
    expect(screen.getByTestId('palette-omnichannel.message_received')).toBeInTheDocument();
    expect(screen.getByTestId('palette-omnichannel.get_contact')).toBeInTheDocument();
    expect(screen.getByTestId('palette-omnichannel.send_message')).toBeInTheDocument();
  });
  ...
  it('always shows the core ai_agent.run action regardless of module state', () => {
    ...
    fireEvent.change(screen.getByTestId('palette-search'), { target: { value: 'ai agent' } });
