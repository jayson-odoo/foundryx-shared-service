...
import { buildListNav, encodeListQuery } from '@/lib/list-context';
...
function countConditions(
  group: { rules: unknown[] } | null | undefined,
): number {
  if (!group) return 0;
  return (group.rules as { kind: string; rules?: unknown[] }[]).reduce(
    (n, r) =>
      n + (r.kind === 'group' ? countConditions(r as { rules: unknown[] }) : 1),
    0,
  );
}
...
export interface ResourceListProps<T extends object> {
  ...
  /**
   * Suppresses the list's own `PageHeader` - for a list embedded inside
   * another record's tab (a form's Templates/Submissions/etc. tab, master-
   * detail) or a chrome-less host, where the PARENT surface already owns the
...
export function ResourceList<T extends object>({
  ...
  const changeView = (v: 'card' | 'list') => {
    ...
    if (typeof window !== 'undefined')
      window.localStorage.setItem(viewStorageKey, v);
  };
  const cardView = canCard && view === 'card';

  const sorting: SortingState = list.sort
    ? [{ id: list.sort.id, desc: list.sort.desc }]
    : [];
  ...
  const showStatusViews =
    config.enableStatusViews !== false && !config.segments;
  ...
  const fixedIds = useMemo(
    () =>
      new Set(
        effectiveColumns
          .filter((c) => c.meta?.reorderable === false)
  ...
  const columnOrder = useMemo(() => {
    const raw = prefs.columnOrder.length
      ? prefs.columnOrder
      : defaultColumnOrder;
    ...
    let m = 0;
    return defaultColumnOrder.map((id) =>
      fixedIds.has(id) ? id : movableOrdered[m++],
    );
  ...
  function handleColumnDragEnd(event: DragEndEvent) {
    ...
    let m = 0;
    const next = defaultColumnOrder.map((id) =>
      fixedIds.has(id) ? id : reordered[m++],
    );
  ...
  // own opt-out sentinel (AC-DLA-14) - so it passes through untouched.
  function buildRowHref(row: T, indexOnPage: number): string {
    const href = config.rowHref(row);
    if (!href || href === '#') return href;
    const globalIndex =
      list.loadedQuery.page * list.loadedQuery.pageSize + indexOnPage;
    return buildListNav(href, {
      ctx,
      i: globalIndex,
      from: config.getRowId(row),
    });
  }
  // `rowHref` (not `onRowClick`) for every navigable list (AC-DLA-29) - the
  ...
  function openRow(row: T) {
    ...
    const href = buildRowHref(row, indexOnPage);
    if (!href || href === '#') return;
    router.push(href);
  }
  ...
  const createButton =
    config.onCreate &&
    (!config.createPermission || can(config.createPermission)) ? (
    ...
    <>
      ...
      >
        <Card>
            ...
            <CardToolbar>
              {selectedRows.length > 0 ? (
                ...
                <div className="flex flex-wrap items-center justify-end gap-2">
                  ...
                  {/* Embedded lists (hideHeader) have no PageHeader actions
                    slot to move Create into, so it stays on the toolbar. */}
          ...
          <CardFooter
            className={cn(
              'justify-center md:justify-between flex-col md:flex-row gap-3',
            )}
          >
