...
function SheetOverlay({ className, ...props }: React.ComponentProps<typeof SheetPrimitive.Overlay>) {
  return (
    <SheetPrimitive.Overlay
      ...
      className={cn(
        'fixed inset-0 z-(--z-modal) bg-black/30 [backdrop-filter:blur(4px)] data-[state=open]:animate-in data-[state=closed]:animate-out data-[state=closed]:fade-out-0 data-[state=open]:fade-in-0',
        className,
      )}
      {...props}
    />
...
const sheetVariants = cva(
  'flex flex-col items-strech fixed z-(--z-modal) gap-4 bg-background p-6 shadow-lg transition ease-(--ease-standard) data-[state=open]:animate-in data-[state=closed]:animate-out data-[state=closed]:duration-(--duration-slow) data-[state=open]:duration-(--duration-slow)',
  {
    variants: {
      side: {
