...
const accordionTriggerVariants = cva(
  'flex flex-1 items-center justify-between py-4 gap-2.5 text-foreground font-medium transition-colors [&[data-state=open]>svg]:rotate-180 cursor-pointer',
  {
    variants: {
      variant: {
        default: '',
      ...
      indicator: {
        arrow: '',
        plus: '[&>svg>path:last-child]:origin-center [&>svg>path:last-child]:transition-[transform,opacity] [&>svg>path:last-child]:duration-(--duration-base) [&[data-state=open]>svg>path:last-child]:rotate-90 [&[data-state=open]>svg>path:last-child]:opacity-0 [&[data-state=open]>svg]:rotate-180',
        none: '',
      },
    },
    defaultVariants: {
...
const accordionContentVariants = cva(
  'overflow-hidden text-sm text-accent-foreground transition-colors data-[state=closed]:animate-accordion-up data-[state=open]:animate-accordion-down',
  {
    variants: {
      variant: {
        default: '',
...
function AccordionTrigger(props: React.ComponentProps<typeof AccordionPrimitive.Trigger>) {
  ...
  return (
    <AccordionPrimitive.Header className="flex">
      ...
      >
        {children}
        {indicator === 'plus' && <Plus className="size-4 shrink-0 transition-transform duration-(--duration-base)" strokeWidth={1} />}
        {indicator === 'arrow' && (
          <ChevronDown className="size-4 shrink-0 transition-transform duration-(--duration-base)" strokeWidth={1} />
        )}
      </AccordionPrimitive.Trigger>
