...
function NavigationMenuTrigger({
  ...
  return (
    ...
    >
      {children}{' '}
      <ChevronDownIcon
        className="relative top-[1px] ms-1 size-3.5 opacity-60 transition ease-(--ease-standard) duration-(--duration-slow) group-data-[state=open]:rotate-180"
        aria-hidden="true"
      />
    </NavigationMenuPrimitive.Trigger>
  );
...
function NavigationMenuIndicator({
  ...
  return (
    <NavigationMenuPrimitive.Indicator
      ...
      // Local stacking nudge above the nav list's own siblings only - not one of
      // the shared named z-scale steps (config.reui.css), so it stays out of the
      // Tailwind `z-[N]` class vocabulary the literal sweep bans.
      style={{ zIndex: 1 }}
      {...props}
    >
