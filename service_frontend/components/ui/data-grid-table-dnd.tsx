...
function DataGridTableDndHeader<TData>({ header }: { header: Header<TData, unknown> }) {
  ...
  return (
    <DataGridTableHeadRowCell
      header={header}
      dndStyle={reorderable ? style : undefined}
      dndRef={reorderable ? setNodeRef : undefined}
    >
...
function DataGridTableDnd<TData>({ handleDragEnd }: { handleDragEnd: (event: DragEndEvent) => void }) {
  ...
  // Only reorderable columns participate in the sortable set; the rest stay put.
  const sortableIds = table
    .getVisibleLeafColumns()
    .filter((c) => c.columnDef.meta?.reorderable !== false)
    .map((c) => c.id);
