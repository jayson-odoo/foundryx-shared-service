...
import { ideaFormPath } from '../ideas/components/paths';
              ...
              {(columns[key] ?? []).map((idea) => (
                <KanbanItem key={idea.id} value={idea.id}>
                  <KanbanItemHandle asChild>
                    <Link href={ideaFormPath(idea.id)} draggable={false}>
...
/**
 * Idea triage board (plan Phase A) — Canny-style Kanban across the lifecycle.
 * Drag a card to another column → status change. Drag within a column → reorder
 * priority (board reading order = priority). A floating ghost + grip handle make
 * the drag legible. Click a card (without dragging) → opens the idea form view.
 * Phase 1 uses the mock service.
 */
