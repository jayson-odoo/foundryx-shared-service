...
Backend was tested across EIGHT HEADs as review-round fix commits landed mid-pass (coordinator
...
- `88106ace` - review round 8 (Defect 5 fix: `task-editor-view.tsx`'s `onCancel` now re-derives
  `httpPreviewedFor` from the baseline task, the same guard the seed effect uses, instead of
  leaving it pointed at whatever a discarded edit's own Test had just proved - re-entering Edit
  after a Cancel and typing the SAVED, already-Tested config back in now enables `Save task`
  with no redundant Test; `routers/http.py`'s `preview_http` now maps `CompanyNotFound` through
  the SAME `_raise` translator `routers/companies.py` uses, closing a bare-500 leak vector on an
  other-tenant/unknown `companyId`; `etl_service.py`'s `preview_http` return type drops the bare
  `Any`; the AC-08-20 seed-path fixture now uses a `resultColumns`-non-empty fixture; the mock
  `previewHttp` only echoes a task when the entity already has a real config row). Re-verified
  independently by a fifth tester pass (round-8 verify, `08-evidence/round8-verify/`), backend
  restarted (killed pid `18339` after confirming its `cwd` was `s37/service_backend`, new pid
  `28393`), frontend rebuilt (`rm -rf .next && npm run build`, clean; killed pid `18628` after
  confirming its `cwd` was `s37/service_frontend`, `npx next start -p 3007`, new pid `28718`)
  - both confirmed via `git rev-parse HEAD` on the running pids' `cwd` before this pass. **Final
  HEAD as of this report: `c1ea9d47`** (round-8 verify commit, containing code commit
  `88106ace`); the report's PASS/FAIL verdicts and suite counts are as of `88106ace` unless a
  line says otherwise.
...
**Defect 5 - FIXED at `88106ace` (round 8).** `onCancel` reset `config`/`sourceKind` back to
the baseline (unedited) task on Cancel, but left `httpPreviewedFor` untouched - so if the
discarded edit had Tested a DIFFERENT path than the saved one, `httpPreviewedFor` stayed
pointed at that discarded, never-saved pair. Re-entering Edit and typing the SAVED,
already-Tested path back in (e.g. after a Cancel + a second Edit, or any edit that lands back
on the exact saved value) found the previewed pair mismatched against the current value and
kept `Save task` disabled, forcing a redundant Test on a configuration the server had already
proved. Found while executing this round's brief flow (Edit -> change path -> Test -> Cancel ->
Discard -> Edit again -> dirty the form without a new Test -> Save must be enabled), not part
of round 7's own fix scope.

Fix: `task-editor-view.tsx`'s `onCancel` now re-derives `httpPreviewedFor` from the baseline
task BEFORE `draft.reset()` - the same `task.resultColumns.length > 0 && task.sourceConfig.
connectionId && task.sourceConfig.path?.trim()` guard the seed-once effect uses - so the next
Edit session starts with the previewed pair pointed at the SAVED, already-proved config, not a
discarded edit's leftover Test. Also in this commit (review round 8, not separately verified as
defects, but re-checked this pass): `preview_http` now maps `CompanyNotFound` through the same
`_raise` translator `routers/companies.py` uses (404, not a bare 500) - see Check 2 below; the
mock `previewHttp` only echoes a task when the entity already has a real config row.

**Independently re-verified this pass** (`08-evidence/round8-verify/README.md`, backend
restarted and frontend rebuilt to `88106ace`): on the `warehouse` task (Mocha logging-sink
company, was `active` with saved path `/location`) - Pause -> Edit -> path A (`/location`) to
path B (`/location/`) -> **Test** (succeeds, `Save task` enabled) -> **Cancel** -> "Discard
changes?" -> **Discard changes** (back to view mode, path A `/location` shown, never the
discarded path B) -> **Edit again, with NO new Test this session** -> typed path B then
immediately back to path A: after path B (differs from the re-derived previewed pair, path A),
`Save task` correctly read `{"disabled": true}` (the "needs a Test at a NEW value" gate held,
no regression); after typing it back to path A (matches the re-derived previewed pair exactly),
`Save task` read **`{"disabled": false}`** with NO new Test call
(`05-dirty-form-save-enabled-no-new-test-{1280,375}.png`) - **Defect 5 CLOSED**. Cancelled the
probe (no dialog appeared this time - the draft was not dirty against baseline by the time
Cancel was clicked); `psql` confirmed the task was untouched (`etl_status = 'paused'`, `path =
'/location'`) throughout the probe. Resumed the task back to `active` afterward
(`06-warehouse-restored-active-1280.png`; `psql` confirms `etl_status = 'active'`, `path =
'/location'`) - fully restored to its exact pre-check state. Zero console errors across the
whole sequence (`agent-browser errors`/`console` both empty at every step).

**Check 2 - Preview 404 mapping, independently re-verified via curl** (`08-evidence/
round8-verify/README.md`): `POST /autocount/http/preview` with a RANDOM UUID `companyId`
(never a real row, `15df0e81-c4fa-4e7a-9431-5f7c3009637d`) against a real no-auth connection
returned `HTTP_STATUS:404`, body `{"detail":"That AutoCount company was not found."}`; the
backend log shows one tenant-scoped `SELECT ... ac_company WHERE tenant_id = ... AND id = ...`
then `ROLLBACK` then the `404 Not Found` access-log line - no traceback, never a bare 500.

**Deferred residuals (not filed as new defects, not regressions of round 8's own fix scope):**
this pass surfaced four narrower edge cases in the round-8 previewed-pair mechanism, recorded
as `BL-SS-206` (Low priority, `documentation/backlogs/backlog.md`): (1) `onCancel`'s re-derive
guard nulls the previewed pair when the saved task's `result_columns` is empty (a first-ever
task, or an endpoint whose preview genuinely returns zero columns), forcing one redundant Test
on a Cancel-then-Edit-back-to-saved flow for that narrower case - the `warehouse` task used in
this pass had a non-empty `result_columns` (its normal, already-activated state) so it did not
exercise this edge; (2) the mock `previewHttp` echoing logic still auto-creates a blank config
row for a never-before-configured entity in one code path, matching the real backend's
`task: null` only for entities that already have a row; (3) `preview_http`'s tenant-scope guard
on `companyId` runs after the outbound HTTP call completes rather than before it, which is
wasted work (not a leak - Check 2 confirms the eventual response is a clean 404) but could be
hoisted earlier; (4) `onCancel` does not call `httpPreview.reset()`, so a discarded endpoint's
column list stays offered in the key/watermark pickers until the next successful Test. None of
these affect the scenario this AC's Save gate is graded against (a task with a non-empty
previewed pair, which is what every activated/active HTTP task in this plan's evidence has).
...
- `documentation/backlogs/backlog.md`: BL-SS-201 (item lookups no Sorento home), BL-SS-202
  ...
  and linked to this plan; BL-SS-081 correctly reads "Superseded (sprint-5/08)". **BL-SS-206**
  (AutoCount HTTP Source tab previewed-pair residuals after round 8, Low priority) added this
  pass - see the "Deferred residuals" note under Defect 5 above.

## Servers left running

- Backend `:8007` (uvicorn, `app.main:app`) at HEAD `c1ea9d47` (pid `28393`, cwd
  `s37/service_backend`, RESTARTED for the round-8 verify pass - killed pid `18339` after
  confirming its `cwd`).
- Frontend `:3007` (`npx next start -p 3007`, pid `28718`) serving a fresh `c1ea9d47` prod
  build (REBUILT for the round-8 verify pass - `rm -rf .next && npm run build` clean, killed
  pid `18628` after confirming its `cwd`).
- `agent-browser --session s37-verify8` closed (plain `close`, never `close --all`) at the end
  of the round-8 verify pass.
...
- `08-evidence/round8-verify/` - round-8 verify pass at HEAD `88106ace` (Defect 5 closure: a
  Cancel-then-re-Edit sequence that dirties the form back to the SAVED, already-Tested config
  now enables `Save task` with no redundant Test; the "needs a new Test at a genuinely new
  value" gate re-confirmed with no regression; the preview 404 mapping independently
  re-verified via curl against a random-UUID `companyId`; the probed task restored to its exact
  starting state), README + `01`-`06` screenshots at 1280 and 375.
