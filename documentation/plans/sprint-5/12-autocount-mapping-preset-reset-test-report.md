...
- Evidence dirs under `documentation/plans/sprint-5/12-evidence/`: `s1-mock/` (S1, mock phase),
  ...
  `e1857aac`), `s3-documents/` (AC-12-27 FAIL pass), `s3-documents/fix/` (AC-12-27 re-run
  after the fix commit, `--session s50c`, fresh build).
...
`sql_db` task's "Configure mapping" to the TASK editor's Mapping tab, which embedded
`MappingEditorBody` without the `ActionMenu` item or the dialog. The fix mounts the SAME action
and the SAME dialog on that tab through one shared `useMappingResetAction`
(`mapping/components/mapping-reset-action.tsx`), used by both surfaces. Re-verified by real
clicks on the tester's own document entity: `12-evidence/s3-documents/fix/`.
...
5. **Real clicks at 375 AND 1280 on a fresh build on correctly-owned lane ports** - PASS for the
   master/HTTP path (`s3-final/`) and, after the fix round, PASS for the document path
   (`s3-documents/fix/`; the original FAIL run is kept in `s3-documents/`) - see AC-12-27.
...
| **AC-12-21** `[FE]` | The action lives on the Mapping tab's existing `ActionMenu`, gated by permission AND `hasPreset` | Product HTTP entity | 1. Mapping page 2. Open `Actions` | Exactly one item, `Reset to preset`; hidden without the permission or without a preset; dirty-guard defers to the shell | Menu enumerated live: `["Reset to preset"]` on BOTH surfaces | **PASS**. Live `s3-final/01-1280-action-menu-reset-to-preset.png` (standalone page) and `s3-documents/fix/03-1280-actions-menu-reset-to-preset.png` + `12-375-...png` (task editor's Mapping tab, after the AC-12-27 fix); edit mode drops the menu on both (`s3-documents/fix/04-1280-edit-mode-actions-hidden.png`). Unit `mapping-editor-view.test.tsx` (5 gating cases incl. `autocount_read` supplier = no preset) and `task-editor-view.mapping-reset.test.tsx` (the same gating on the tab) |
...
| **AC-12-27** `[E2E]` | Document entities are resettable, header rows only | A `sales_order` task on a `sql_database` company | Sidebar AutoCount -> Companies -> the DB company -> Entities -> Sales order `Actions` -> Configure mapping -> expect `Actions` -> `Reset to preset` | "Reset to preset" is OFFERED; the dialog lists HEADER rows only; apply leaves every line-scope row untouched | Company + task + a product-seeded 11-header / 14-line mapping provisioned through the product's own routes. **The mapping editor an operator reaches offers NO reset at all**: `Configure mapping` sends a `sql_db` task to `/entities/sales_order?tab=mapping` (`task-editor-view.tsx`), which embeds `MappingEditorBody` without the `ActionMenu` item or `MappingResetDialog`; the standalone page that has them is linked only for non-`sql_db`. Controls present at both widths: `Edit`, the five tabs, `Simulate mapping` - nothing else. Every document entity is `sql_db` (no document in `HTTP_PRESETS`), so the gap is total | **PASS after the fix round.** First pass FAILED (evidence `12-evidence/s3-documents/`, `02`/`03`/`04` + README) with the backend half already correct on that same live entity: `hasPreset: true`; dry run = 11 rows, all header, no line canonical field, `removed: []` (`05-dry-run-header-only.json`); a real apply leaves the 14 line rows byte-identical incl. an operator customisation (`06`/`07`). The fix mounts the SAME `ResourceAction` + `MappingResetDialog` on the task editor's Mapping tab via one shared `useMappingResetAction`. Re-run by real clicks on the same document entity, fresh prod build: `Actions` -> `Reset to preset` present (and hidden in edit mode), dialog lists the 11 HEADER rows only, apply toasts and hydrates from the returned view with no second `GET .../mapping`, and the 14 line rows `diff` EMPTY across the apply. Evidence `12-evidence/s3-documents/fix/` (10 shots + 3 data artefacts + README). Unit: `task-editor-view.mapping-reset.test.tsx` (7 cases) |
...
1. **AC-12-27 - "Reset to preset" unreachable for every document entity** - found FAIL by this
   pass, **FIXED in the same lane** (commit "fix(autocount): sprint-5/12 - Reset to preset
   reachable from the task editor Mapping tab (AC-12-27)"). Not a backend bug and not a
   component bug: a routing split in
   `service_frontend/app/(protected)/autocount/companies/components/company-detail-view.tsx`
   (`onConfigureMapping`) sends a `sql_db` task to the task editor. The fix keeps that routing
   and mounts the SAME action + dialog on the task editor's Mapping tab through one shared
   `useMappingResetAction`, used by the standalone page too. The unit suite could not catch the
   original gap because it rendered `MappingEditorView` directly; the new
   `task-editor-view.mapping-reset.test.tsx` covers the other surface. Re-verified by real
   clicks: `12-evidence/s3-documents/fix/`.
