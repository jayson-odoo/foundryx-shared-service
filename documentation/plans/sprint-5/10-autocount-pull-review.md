...
| D35 (hotfix 2026-09-21, `fix/autocount-pull-activate-gate`) | `EtlService.activate_task`'s failed-row gate now applies ONLY to `delivery_mode == push`; pull-mode Activate ignores a consumer-side dry-run failure count (the Activate tab instead shows a warning banner naming the consumer's own review page) | R6 already ruled "products never block Confirm" for the CONSUMER's review page, but `activate_task` still blocked FOUNDRYX's own Activate on any failed count regardless of mode - a prod product task sat stuck in Draft on 4 consumer-side ref collisions it can never fix locally. Push is untouched: it is still Foundryx that delivers those rows there (the gate reads the sink dry-run summary, not the snapshot's excludedRows - same consumer-side class); browser evidence waived for this text-only Alert prod hotfix, owner-accepted |
