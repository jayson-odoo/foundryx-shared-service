...
### AC-BI-24c — extraction emits EVERY field (completeness), + captured summary + prompt-to-generate [BE][FE][T]
*(Added 2026-07-22 from live user feedback — a 6/6-captured grill generated a BR with business_goal/scope/constraints blank; the model silently omitted synthesized fields.)*
- **Given** Generate runs the extraction, **then** the extraction output schema marks **all target keys required** and the directive demands a value for EVERY field — *"Return a value for every field. For a field discussed across several turns, synthesize the final agreed answer. Only leave a field an empty string if it was genuinely never discussed; NEVER invent."* So a field the transcript established (even across turns, or a negative like "no constraints") is emitted, not silently skipped. Partial emit still holds — a genuinely-ungrounded field comes back blank (validated `enforce_required=False`), never invented.
- **Given** the S3 runaway fix (`thinkingBudget=0`), **then** if completeness alone (required schema + directive) still misses synthesized fields under real Gemini, a **bounded** thinking budget for the extraction call (not 0, not unlimited — verify no runaway) is the fallback. The fix MUST be **live-verified against real Gemini**: a full 6-field grill → Generate → all 6 fields populated on the Details tab.
- **Given** the grill surface, **then** it shows a **running captured summary** — each target field with the value the conversation has established so far (not just "N of M"), updating per turn. Foolproof-UI: values only, no how-to copy.
- **Given** the user asks to generate in natural language ("generate it", "create the BR", "that's enough"), **then** the turn's structured output carries a **`generateSignal`** the agent sets when it detects that intent; the **app** (not the model) fires the existing Generate call — the model SIGNALS, the app ACTS, so the model still has NO side-effect tool (D22-A preserved). The explicit **Generate button stays** (the always-available foolproof control); the NL path is additive. Generate still only writes `answers_json` on a draft — never promotes.
