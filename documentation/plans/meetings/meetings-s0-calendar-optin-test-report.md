...
**Branch:** `sprint-5/meetings-s0-calendar-optin` · **Date:** 2026-08-25
...
|---|---|---|
| Meetings backend | `python -m pytest tests/test_meetings_{scaffold,api,calendar_sync,connections,jobs}.py -q` | `47 passed, 1 warning in 23.54s` |
| Full backend | `python -m pytest -q` | `35 failed, 1780 passed, 18 deselected, 186 warnings in 1091.12s` |
...
| **AC-S0-5** `meet_bot` connection saved, no live test | PASS (T) | `::test_meet_bot_is_offered_with_a_secret_password`, `::test_meet_bot_saves_without_a_live_test`, `::test_meet_bot_test_does_not_pretend_to_have_signed_in`, `::test_a_tenant_can_hold_both_kinds_at_once`. |
| **AC-S0-6** master toggle, off by default, copy-free | PASS (T + B) | API: `test_meetings_api.py::test_optin_is_off_by_default`, `::test_optin_can_be_flipped_both_ways`. UI: `my-meetings-view.test.tsx` "AC-S0-6/9". Browser: the page carries a page title, one switch and the list - no instructional copy anywhere. |
| **AC-S0-7** opted-in user's next 14 days with title/start/end/organiser/attendee count/platform/opt-out switch, within 60 s | PASS (T + B) | Fields: `test_meetings_api.py::test_events_carry_everything_the_row_renders`. Link recognition: `test_meetings_calendar_sync.py::test_meet_zoom_and_teams_links_are_all_mirrored`, `::test_an_event_with_no_conference_link_is_not_mirrored`. The 60 s path: `test_meetings_jobs.py::test_the_beat_tick_enqueues_one_job_per_due_tenant` plus the `meetings.calendar_sync_due` beat entry at `schedule: 60.0`. Browser: three real rows (Meet / Zoom / Teams) rendered from Postgres with every field visible. **Not verified:** the 60 s figure end to end against a live Celery beat - only the tick's enqueue behaviour and the schedule value are tested. |
...
- **Fresh path** (`upgrade head`): ten tables + `alembic_version_meetings` created, revision id
  `0001_meetings_init` (18 chars, inside the 32-char limit), the `pg_trgm` extension and the
  `ix_meetings_segments_text_trgm` GIN index present, `starts_at` is `timestamp with time zone`, all
  three unique indexes present, and a second `upgrade` is a clean no-op.
...
**This found a real bug the SQLite suite could not see:** the migration built from
`MeetingsBase.metadata` without importing `models`, so the metadata was empty, `create_all` was a
silent no-op, and the migration only failed later on the trigram index. Fixed by importing
`modules.meetings.models` inside the migration.
...
- **A real Google Workspace.** Every calendar test drives a scripted `CalendarSource`. The
  `google_dwd` adapter's own request shaping (`events().list` params, pagination, the 410 -> 
  `SyncTokenInvalid` mapping, `parse_event`) has **no test and has never run against Google.** This
  is the single largest untested surface in the slice and is the first thing to exercise when a real
  tenant is onboarded.
