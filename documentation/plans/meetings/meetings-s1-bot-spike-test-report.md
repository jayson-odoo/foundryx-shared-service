...
| AC-S1-6 speaker attribution | **pass (via captions)** | Run 7: bot turns on Meet captions; 20 `caption` events, every one `speaker: "Teh Jayson"` with the spoken text. Names guaranteed by Meet itself. The DOM speaking-indicator route was dropped. |
...
- Captain decision 2026-08-25: keep `jayson@foundryx.my` as the bot for now. AC-S1-10 therefore needs 5 meetings hosted by someone else (personal or colleague accounts); 2 of 5 done (`kfp-sofm-cnr`, `pxd-kacm-hqg`).
- AC-S1-4 password re-login (needs an account without 2SV).
