...
| M12 | **STT = self-hosted Whisper.** Amended 2026-08-25: pilot runs **on the Mac Mini** (`mlx-whisper` on Metal, ~3-5 min per meeting-hour, zero cost); moves to a GPU VM or Modal when volume or the prod move demands it, behind the same `SttProvider` adapter. **Amended 2026-09-01 (S3 code-switch fix):** model flipped from `large-v3-turbo` to non-turbo `large-v3-mlx` - the turbo variant's language head missed a Chinese chunk under the chunked per-chunk detection design, see `PLAN-meetings-s3-stt.md` section 8; speed gate relaxed to ~15 min per meeting-hour (measured 5.5x realtime live) - correctness beat speed, GPU/Modal remains the speed escalation. **Speaker names come from the Meet caption timeline the bot records** (proven S1 run 7), aligned to Whisper timestamps; pyannote diarization only if captions prove insufficient. Deepgram stays the configured fallback. |
