...
| M12 | **STT = self-hosted Whisper.** Amended 2026-08-25: pilot runs **on the Mac Mini** (`mlx-whisper`, `large-v3-turbo` on Metal, ~3-5 min per meeting-hour, zero cost); moves to a GPU VM or Modal when volume or the prod move demands it, behind the same `SttProvider` adapter. **Speaker names come from the Meet caption timeline the bot records** (proven S1 run 7), aligned to Whisper timestamps; pyannote diarization only if captions prove insufficient. Deepgram stays the configured fallback. |
