...
**Date:** 2026-06-06 · **Stack:** Next :3001 (production build) → FastAPI :8001 → Postgres (native) · **Spec:** `service_frontend/e2e/branding.spec.ts` · **Result: 3 / 3 PASSED** (4.5s, fullyParallel)
...
| **Expected** | Upload toasts + card flips to Replace; save toasts; the app re-themes live (`--foundryx-primary` on `<html>`); tab title = tenant name; template prefilled with the saved override; sign-in page shows the tenant logo (served from `/public/branding/{slug}/asset/logo`), the slogan, a purple Sign In button - and NO Foundryx tagline. |
| **Actual** | As expected. |
...
## Scenario 2 - Unbranded tenant gets stock Foundryx branding
...
| **Expected** | Foundryx tagline "Bringing Events to Life." visible; tab title "Foundryx EMS". |
| **Actual** | As expected. |
...
| **Expected** | Save succeeds via the operator endpoints (`tenants.manage_branding`); the OPERATOR's own console keeps stock theming (no `--foundryx-primary` override on its `<html>` - the edit targeted the other tenant); the tenant's sign-in page shows the new slogan + its name in the tab. |
| **Actual** | As expected. |
