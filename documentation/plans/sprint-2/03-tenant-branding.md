...
Dreamz EMS is multi-tenant SaaS; tenants need their own identity on the surfaces their users see: sign-in page, browser tab, app header, and the system color theme. Today everything is hardcoded Dreamz (orange `#FF5A00`, `AuthBrandPanel` defaults, static favicon). The foundation must scale: many tenants, each with distinct light+dark themes, served from one deployment.

### Locked design decisions (from grilling)

1. **Stored theme format = curated flat JSON.** One document, `light` + `dark` sections, **whitelisted variable names only** (all Layer-1 `--dreamz-*` primitives: primary family, greys, status colors, light/dark surfaces), values = valid CSS colors only. Backend validates names against the whitelist and color syntax; anything else → 422. Partial overrides allowed — only changed vars are stored and emitted; everything else falls through to `dreamz-tokens.css` defaults.
...
3. **Injection = backend-generated CSS + `<link>`.** Public endpoint `GET /public/branding/{slug}/theme.css` renders `:root { ... }` + `.dark { ... }` from stored JSON (overridden vars only). Next root layout (server component) derives slug from host (`lib/tenant.ts deriveTenantSlug`) and emits the `<link>` **after** `dreamz-tokens.css` so tenant values win. ETag + `Cache-Control` + `?v={version}` busting. Render-blocking link = no flash of Dreamz orange.
4. **Asset set:** logo (used in 3 places: sign-in brand panel, app header top-left, browser-tab fallback), **optional favicon** (square; falls back to logo — wide wordmarks are unreadable at 16px), **optional brand-panel illustration**, slogan (plain text). Empty slogan = nothing rendered (no Dreamz fallback on a branded tenant).
...
6. **Data model = `tenant_branding` 1:1** (no two-tier fork machinery): `tenant_id` PK/FK, `slogan`, `logo_path`, `favicon_path`, `illustration_path`, `tokens_json` JSONB, `version` int (bumped on every save — busts CSS + asset URLs), timestamps. No row = pure Dreamz defaults.
...
8. **Browser tab:** title = **tenant name** (no separate product-name field), favicon = uploaded icon; via Next `generateMetadata` reading host → public branding endpoint. Default tenant / no branding = "Dreamz EMS".
9. **Unknown slug on every public branding endpoint → 200 with Dreamz defaults**, never 404 — no tenant-enumeration signal, sign-in page always renders.
10. **Platform tenant keeps hardcoded Dreamz branding** (the console IS the product).
...
`app/branding/token_whitelist.py` — the canonical ordered list of variable names (mirrors Layer-1 primitives in `css/dreamz-tokens.css`), each with: key (e.g. `primary-active`), CSS var name (`--dreamz-primary-active`), group (`brand|grey|status|surface`), label. Frontend picker groups + template generation + backend validation all derive from this one list (exposed via `GET /branding/template`). Add a themeable var = add one entry.

## Backend

### Public (no auth — consumed pre-login + by `<link>`/`<img>`/metadata)
- `GET /public/branding/{slug}` → JSON `{tenantName, slogan, logoUrl, faviconUrl, illustrationUrl, version}` (URLs absolute, version-stamped). Unknown slug → Dreamz defaults.
...
- **Root layout** (`app/layout.tsx`, server): derive slug from host → fetch `GET /public/branding/{slug}` (cached per request) → emit theme `<link>`; `generateMetadata` → title = tenant name, icon = favicon URL. Failure → Dreamz defaults (never block render).
...
- **Header top-left**: replace hardcoded logo with branding logo (fallback Dreamz).
