...
- **Mailbox rig:** `python -m aiosmtpd -n -l localhost:1025 -c aiosmtpd.handlers.Mailbox /tmp/foundryx-e2e-mailbox` — the maildir needs `tmp/new/cur` subdirs created up front, and a leftover plain `aiosmtpd` from the plan-09 session had to be replaced (port owner check).
