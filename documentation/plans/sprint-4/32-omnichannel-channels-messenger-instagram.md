...
| D-A7-9 | `get_adapter` moves into an `adapters/__init__.py` REGISTRY; `adapters.whatsapp_cloud.get_adapter` stays as a thin re-export | Eight modules import it from `whatsapp_cloud`. Rewriting those imports inside a slice that is already refactoring the send path is pure risk for zero behaviour. The re-export is one line and the follow-up import sweep is BL-SS-115 |
...
| D-A7-21 | Broadcasts (A4) are unchanged and remain WhatsApp-template-only; a phone-less Messenger contact simply resolves as `skipped/no_identity` for a WhatsApp broadcast | A4 resolves recipients through `contact_channel_identities` on the CHOSEN channel, so it is already channel-correct. Messenger broadcasts would need Meta's tag policy, which D-A7-7 rules out. BL-SS-117 |
...
## 8. Backlog candidates (register on close; ids reserved from BL-SS-110)

| Id | Title | Priority |
|---|---|---|
| BL-SS-110 | Messenger / Instagram message templates (generic, button, receipt, media templates) as a first-class composer + workflow surface | Medium |
| BL-SS-111 | Persistent menu, ice breakers and greeting text per Messenger / Instagram channel (a Profile-tab equivalent for these types) | Medium |
| BL-SS-112 | Handover protocol (pass thread control to or from the Page inbox / another app) | Low |
| BL-SS-113 | Cross-channel contact merge: fuse a WhatsApp contact and a Messenger / Instagram contact into one profile, moving identities (pairs with G24 / B2) | High |
| BL-SS-114 | Message tags beyond `HUMAN_AGENT` (`CONFIRMED_EVENT_UPDATE`, `POST_PURCHASE_UPDATE`, `ACCOUNT_UPDATE`) and one-time notifications, if and when Meta's roadmap settles | Low |
| BL-SS-115 | Import sweep: point the eight `from ..adapters.whatsapp_cloud import get_adapter` sites at `adapters/__init__.py` and drop the re-export | Low |
| BL-SS-116 | Instagram Login (IG-direct, no linked Facebook Page) as a second connect topology | Medium |
| BL-SS-117 | Broadcasts on Messenger / Instagram once a compliant outbound mode exists (pairs with BL-SS-114) | Low |
| BL-SS-118 | Instagram story replies, story mentions and media shares as first-class message types instead of `UNSUPPORTED` placeholders | Medium |
| BL-SS-119 | Outbound reactions on Messenger / Instagram (inbound already lands on the existing reaction path) | Low |
| BL-SS-120 | Per-page rate-limit awareness (Meta's calls-per-page budget) surfaced in the channel Configuration tab, pairing with BL-SS-082 | Low |
| BL-SS-121 | A channel CATALOG screen (respond.io's card grid) once more than three types ship - A7a deliberately uses a `SearchSelect` step instead | Low |
| BL-SS-122 | `channel_type` as a DB-level enum plus a migration for the pruned `DOUYIN` / `XIAOHONGSHU` values, should any row ever carry them | Low |
...
- **F2 - an AUTOMATED reply is refused on Messenger / Instagram once the 24h window closes
  ...
  answer is a template (BL-SS-110), not the human-agent tag.
- **F3 - no cross-channel merge (D-A7-4).** The same person on WhatsApp and on Messenger is two
  contacts until BL-SS-113 ships. This is visible in the Contacts list, in segments and in the
...
- **F5 - the connect wizard gains a `SearchSelect` step, not respond.io's card catalog** (BL-SS-121).
...
- **F6 - `DOUYIN` and `XIAOHONGSHU` are removed from the frontend `ChannelType` union** (D-A7-20).
  ...
  any stored row somehow carries one it renders as an unknown type; BL-SS-122 covers the enum.
