...
Full-fidelity history for one contact — **every message type** (text, media, interactive, location, contacts, template, reactions, replies). Read-only (does **not** mark the thread read).
...
```jsonc
{
  ...
  "message": {
    "type": "image",                    // text|image|video|audio|voice|document|sticker|
                                        //   interactive|location|contacts|template|unsupported
  ...
  "status": [                           // delivery receipts, newest state; EMPTY for inbound
    { "value": "delivered", "timestamp": 1783508205, "message": null }
  ],
  "sender": {
...
* **`timestamp` is the time key.** `status[]` is built from delivery receipts and is **always empty for inbound** — never order on it.
* **`status[].value`** ∈ `pending | sent | delivered | read | failed`. On `failed`, the reason is in `status[].message`.
* **`message.type`** is lowercase. `voice` and `audio` are distinct types — a WhatsApp voice note is `"voice"`, so you do not need a separate boolean to tell them apart.
...
| `errorCode` / `errorMessage` | `status[].message` |
