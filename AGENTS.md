...
python -m pytest -q                            # pytest + httpx; conftest = in-memory sqlite, seeded ONCE per WORKER PROCESS then byte-copied per test (template-DB pattern); migrations INVISIBLE to it. Bare `pytest` stays serial (~3s/file); full suite = `python -m pytest -q -n auto --dist loadfile` (parallel, ~3 min on 10 cores) - xdist is opt-in, not baked into addopts.
```
