...
Dreamz is the Tech Provider: Embedded Signup hands us an auth code which we
exchange for a permanent system-user token against the ONE Dreamz Meta app.
...
class WhatsAppCloudAdapter:
    ...
    def fetch_waba_details(self, credentials: Dict[str, Any], waba_id: str) -> Dict[str, Any]:
        ...
        if not self._configured or credentials.get("dev") or not waba_id:
            return {"name": "Dreamz Events (dev sandbox)"}
        client = self._http()
        try:
            resp = client.get(
