...
Which theme variables a tenant may override, their Dreamz defaults, validation
and CSS generation. Mirrors the Layer-1 ``--dreamz-*`` primitives in the
frontend's ``css/dreamz-tokens.css``; the frontend keeps a display mirror in
...
- ``-transparent`` variants — derived (base color @ 0.2 alpha), see
  ``derive_transparent``;
- ``--dreamz-white`` / ``--dreamz-black`` — true constants.
"""
import re
...
# Dreamz defaults — MUST mirror css/dreamz-tokens.css Layer 1 exactly.
DREAMZ_DEFAULTS: TokensDoc = {
    "light": {
        "primary": "#ff5a00",
...
def validate_tokens(doc: object) -> Tuple[Optional[TokensDoc], List[str]]:
    ...
    dropped). Values equal to the Dreamz default normalize away so the stored
    ...
    for theme in ("light", "dark"):
        ...
        for key, value in section.items():
            ...
            if normalized != normalize_hex(DREAMZ_DEFAULTS[theme][key]):
...
def build_template(tokens: Optional[TokensDoc]) -> TokensDoc:
    ...
    for theme in ("light", "dark"):
        ...
        for key, _ in TOKEN_DEFS:
            out[theme][key] = overrides.get(key, DREAMZ_DEFAULTS[theme][key])
    return out


def tokens_to_css(tokens: Optional[TokensDoc]) -> str:
    """``:root { … } .dark { … }`` — overridden vars only (+ derived
    transparents). Defaults stay in the frontend's dreamz-tokens.css."""
    if not tokens:
        return ""
    ...
    def block(selector: str, theme: str) -> str:
        ...
        for key, _ in TOKEN_DEFS:
            ...
            if not value:
                continue
            lines.append(f"  --dreamz-{key}: {normalize_hex(value)};")
            if key in TRANSPARENT_BASES:
                lines.append(f"  --dreamz-{key}-transparent: {derive_transparent(value)};")
        return f"{selector} {{\n" + "\n".join(lines) + "\n}" if lines else ""
