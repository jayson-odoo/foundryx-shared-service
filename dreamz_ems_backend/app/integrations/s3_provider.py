...
def _http_get(url: str, timeout: float) -> bytes:
    ...
    """
    request = urllib.request.Request(url, headers={"User-Agent": "DreamzEMS-StorageProbe/1.0"})
...
class S3CompatibleAdapter:
    ...
    def run_probe(self) -> TestResult:
        ...
        probe_body = f"dreamz-probe-{uuid.uuid4().hex}".encode()
        key = ""
        try:
            key = self.save("dreamz-probe/probe", probe_body, "text/plain")
            if self.cdn_base_url:
