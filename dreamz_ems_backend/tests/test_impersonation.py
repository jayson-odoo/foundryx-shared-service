...
def test_start_requires_impersonate_permission(client, session_factory):
    ...
    _make_user(session_factory, "limited@dreamz.io", "pw12345678", perm_keys=["roles.read"])
    target = _make_user(session_factory, "target@dreamz.io", "pw12345678", perm_keys=["roles.read"])
    headers = _auth(client, email="limited@dreamz.io", password="pw12345678")
...
def test_admin_can_start_and_get_current(client, session_factory):
    target = _make_user(session_factory, "t1@dreamz.io", "pw12345678", perm_keys=["roles.read"])
    res = client.post("/impersonation/start", headers=_auth(client), json={"targetUserId": target})
    assert res.status_code == 200
    body = res.json()
    assert body["targetUser"]["email"] == "t1@dreamz.io"
...
def test_cannot_impersonate_another_impersonator(client, session_factory):
    other = _make_user(
        session_factory, "admin2@dreamz.io", "pw12345678", perm_keys=["users.impersonate"]
    )
...
def test_cannot_impersonate_more_privileged_user(client, session_factory):
    ...
    target = _make_user(
        session_factory,
        "privileged@dreamz.io",
        "pw12345678",
        perm_keys=["roles.read", "roles.delete"],
    )
    headers = _auth(client, email="support@dreamz.io", password="pw12345678")
    res = client.post("/impersonation/start", headers=headers, json={"targetUserId": target})
    assert res.status_code == 403
...
def test_header_swaps_effective_user_only_with_active_session(client, session_factory):
    ...
    target = _make_user(session_factory, "t2@dreamz.io", "pw12345678", perm_keys=["roles.read"])
    ...
    assert me["email"] == "t2@dreamz.io"


def test_stop_ends_session(client, session_factory):
    target = _make_user(session_factory, "t3@dreamz.io", "pw12345678", perm_keys=["roles.read"])
    client.post("/impersonation/start", headers=_auth(client), json={"targetUserId": target})
