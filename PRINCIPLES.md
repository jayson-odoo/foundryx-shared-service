...
The slim, always-true rules. `CLAUDE.md` is the slim index and `documentation/engineering/<topic>.md` the detailed per-engine reference; **this file governs - on conflict, this wins.** Keep it short; deep detail belongs in `CLAUDE.md` / `documentation/`.
...
- **No `<style>` tags / no raw CSS in components or pages - Tailwind/Metronic utilities only. The ONLY sanctioned CSS files are `css/config.reui.css` (tokens), `css/foundryx-tokens.css` (brand), `css/styles.css` (utilities + the three accessibility preference blocks) and `css/demos/demo1.css` (shell); a rule that belongs to one component goes on the component as utilities.** Brand source of truth = `css/foundryx-tokens.css` + the TokensStudio JSON.
...
DB query / raw SQL in a router · a React component calling axios/fetch directly · `any` types · No `<style>` tags / no raw CSS in components or pages - Tailwind/Metronic utilities only. The ONLY sanctioned CSS files are `css/config.reui.css` (tokens), `css/foundryx-tokens.css` (brand), `css/styles.css` (utilities + the three accessibility preference blocks) and `css/demos/demo1.css` (shell); a rule that belongs to one component goes on the component as utilities. · a module ALTERing core `public` tables · a "done" slice still serving a mock · a new column/engine with no backfill for existing rows/tenants · hardcoded lookup of a tenant-editable key · a new permission with no existing-tenant grant path.

**Design-language hard-fails (plan 23, `docs/reference/design-language.md` section 3):** `transition-all` (or `transition: all`) · `transform: scale(0)` as an entrance · `ease-in` on any entrance · a raw `cubic-bezier` outside `config.reui.css` · motion on a keyboard-initiated action · a new destructive confirm dialog outside the named typed-confirm carve-outs (module uninstall, tenant purge single-row + bulk, Documents > Shares bulk revoke) · a raw `<table>` outside the two content allowlist entries (`form-renderer/table-field.tsx`, `email-editor/block-view.tsx`) · an unlabelled icon button (no `aria-label`/`sr-only` text) · a bare `Loading...`/`Loading…` string · a direct `sonner` import outside `lib/toast.ts`'s three sanctioned files · a `text-[Npx]` class outside the demo1 layout · `PRESSED_CLASS` (or any `active:scale-*`) on a `cursor-grab` hold element (a drag is a HOLD, not a press - it would sit compressed for the whole gesture and compound with the drag's own transform).
